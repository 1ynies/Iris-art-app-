...
class IrisEditingScreen extends StatefulWidget {
  ...
  final List<String> imageUrls; // These are the RAW originals
...
class _IrisEditingScreenState extends State<IrisEditingScreen> {
  ...
  void initState() {
    super.initState();
    _projectImages = widget.imageUrls
        .map((path) => IrisImage(id: UniqueKey().toString(), imagePath: path))
        .toList();
  }
  ...
  Future<void> _navigateBack() async {
    ...
    if (shouldLeave) {
      // ✅ FIX: Return widget.imageUrls (Originals) instead of _projectImages
      // This ensures Image Prep 1 receives the raw images again.
      context.goNamed(
        'image-prep',
  ...
  Future<void> _cropAndSaveIris() async {
    try {
      ...
      final double innerHeight = innerWidth * _ovalRatio;
      ...
      final double cropSize = outerWidth > outerHeight ? outerWidth : outerHeight;
      ...
      final ui.Image processedImage = await picture.toImage(cropSize.toInt(), cropSize.toInt());
      final ByteData? pngBytes = await processedImage.toByteData(format: ui.ImageByteFormat.png);

      if (pngBytes != null) {
        final directory = await getApplicationDocumentsDirectory();
        final String newPath = '${directory.path}/cropped_${DateTime.now().millisecondsSinceEpoch}.png';
        final File newFile = File(newPath);
        await newFile.writeAsBytes(pngBytes.buffer.asUint8List());

        setState(() {
          IrisImage updatedImg = _activeImage.copyWith(imagePath: newPath, isCirclingDone: true);
          _projectImages[_selectedImageIndex] = updatedImg;
          _currentStep++;
        });

        ToastService.showSuccess(context, title: "Success", message: "Iris extracted successfully");
      }
    } catch (e) {
  ...
  void _resetSelection() {
    setState(() {
      ...
      ToastService.showSuccess(context, title: "Reset", message: "Selection reverted.");
    });
  }
  ...
  void _applyCurrentStep() async {
    if (_currentStep == 0) {
      ...
      ToastService.showSuccess(context, title: "Saved", message: "Changes applied.");
    }
  }
  ...
  Future<void> _pickImage(BuildContext context) async {
    ...
    if (result != null && result.files.single.path != null) {
      ...
      if (_projectImages.length >= 6) {
        if (mounted) {
          ToastService.showError(context, title: "Limit Reached", message: "You can only upload a maximum of 6 images.");
        }
        return;
      }
      ...
      if (mounted) {
        setState(() {
          _projectImages.add(IrisImage(id: UniqueKey().toString(), imagePath: filePath));
        });

        context.read<ProjectHubBloc>().add(UploadImageTriggered(projectId: widget.session.id, imagePath: filePath));
        ToastService.showSuccess(context, title: "Image Added", message: "New iris added to queue.");
      }
    }
  }
      ...
      floatingActionButton: Opacity(
        ...
        child: SizedBox(
          ...
          child: GlobalSubmitButtonWidget(
            ...
            onPressed: () {
              if (!_allImagesDone) {
                ToastService.showError(context, title: "Pending Images", message: "Please finish editing all images in the queue first.");
              } else {
                final currentPaths = _projectImages.map((e) => e.imagePath).toList();
                context.go('/image-prep-2', extra: {'session': widget.session, 'imageUrls': currentPaths});
              }
            },
          ),
        ),
      ...
      case 1:
        return FlashCorrectionView(
          ...
          onImageUpdated: (newPath) {
            setState(() {
              IrisImage updated = _activeImage.copyWith(imagePath: newPath, isFlashDone: true);
              _projectImages[_selectedImageIndex] = updated;
            });
          },
        );
  ...
  Widget _buildSimpleSlider({required String label, required double value, required Function(double) onChanged, String? overrideDisplay}) {
    return Container(
        ...
        children: [
          Row(
            ...
            children: [
              ...
              Text(overrideDisplay ?? "${(value * 100).toInt()}%", style: const TextStyle(color: Colors.blueAccent, fontSize: 12)),
            ],
          ),
          SliderTheme(
            data: SliderTheme.of(context).copyWith(trackHeight: 4, thumbShape: const RoundSliderThumbShape(enabledThumbRadius: 6)),
            child: Slider(value: value.clamp(0.0, 1.0), onChanged: onChanged, activeColor: Colors.blueAccent),
          ),
        ],
      ),
    );
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          const Gap(32),
          _buildStepItem(2, "Color Adjustment", 'assets/Icons/color_swatch_solid.svg'),
        ],
      ),
    );
  }
  ...
  Widget _buildStepItem(int index, String title, String iconpath) {
    ...
    bool isAccessible = index == 0 || (index == 1 && _activeImage.isCirclingDone) || (index == 2 && _activeImage.isFlashDone);
    Color color = isActive ? Colors.blueAccent : (isAccessible ? Colors.grey : Colors.white12);
    return InkWell(
      ...
      child: Column(
        children: [
          Row(
            children: [
              SvgPicture.asset(iconpath, color: isActive ? Colors.blueAccent : color, width: isActive ? 20 : 15, height: isActive ? 20 : 15),
              const Gap(8),
              Text(title, style: TextStyle(color: isActive ? Colors.white : color, fontWeight: isActive ? FontWeight.bold : FontWeight.normal)),
            ],
          ),
          const Gap(8),
          Container(height: 2, width: 200, color: isActive ? Colors.blueAccent : Colors.transparent),
        ],
      ),
    );
  }
      ...
      child: Column(
        ...
        children: [
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              const Text("QUEUE", style: TextStyle(color: Colors.grey, fontWeight: FontWeight.bold, letterSpacing: 1.2)),
              Text("${_projectImages.where((i) => i.isFullyEdited).length}/${_projectImages.length} done", style: const TextStyle(color: Colors.grey, fontSize: 12)),
            ],
          ),
          const Gap(16),
          Expanded(
              ...
              itemBuilder: (context, index) {
                if (index == _projectImages.length) {
                  return InkWell(
                    ...
                    child: DottedBorder(
                      color: const Color(0xFF687890), strokeWidth: 2, dashPattern: const [8, 4], borderType: BorderType.RRect, radius: const Radius.circular(12),
                      child: Container(
                        height: 100, width: double.infinity, decoration: BoxDecoration(color: Colors.white.withOpacity(0.02), borderRadius: BorderRadius.circular(12)),
                        child: Column(
                          mainAxisAlignment: MainAxisAlignment.center,
                          children: [
                            Container(width: 48, height: 48, decoration: const BoxDecoration(color: Color(0xFF2A3441), shape: BoxShape.circle), padding: const EdgeInsets.all(12), child: const Icon(Icons.cloud_upload_outlined, color: Color(0xFF94A3B8))),
                          ],
                        ),
                      ),
                    ),
                ...
                Color borderColor = isSelected ? Colors.blueAccent : (isDone ? Colors.green : Colors.transparent);
                Color badgeColor = isSelected ? Colors.blueAccent : (isDone ? Colors.green : Colors.grey.withOpacity(0.8));
                String badgeText = isSelected ? "EDITING" : (isDone ? "DONE" : "PENDING");

                return GestureDetector(
                  onTap: () => _switchImage(index),
                  child: Container(
                    height: 200,
                    decoration: BoxDecoration(border: Border.all(color: borderColor, width: (isSelected || isDone) ? 2 : 0), borderRadius: BorderRadius.circular(8), color: const Color(0xFF2A3441)),
                    child: Stack(
                      fit: StackFit.expand,
                      children: [
                        ClipRRect(borderRadius: BorderRadius.circular(6), child: ColorFiltered(colorFilter: (!isSelected && !isDone) ? const ColorFilter.mode(Colors.black54, BlendMode.darken) : const ColorFilter.mode(Colors.transparent, BlendMode.dst), child: Image.file(File(img.imagePath), fit: BoxFit.cover))),
                        Positioned(top: 8, right: 8, child: Container(padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2), decoration: BoxDecoration(color: badgeColor, borderRadius: BorderRadius.circular(4)), child: Text(badgeText, style: const TextStyle(color: Colors.white, fontSize: 8, fontWeight: FontWeight.bold)))),
                      ],
                    ),
                  ),
                );
